...
- Hero Exception: the Hero section deliberately breaks two rules above for visual hierarchy on the page's highest-impact section. Its primary CTA and accent underline use the accent color at rest, not only on hover. Its secondary CTA and code block use a single 1px border. Every other component keeps the rules above unchanged.
